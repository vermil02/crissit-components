...
## 넘길 때는 두 파일만 준다 (2026-09-10)

**개발·다른 AI 세션에는 카탈로그를 주지 않는다.** 148KB 라서 읽는 것만으로 지친다.

| 무엇 | 크기 | 몫 |
|---|---|---|
| `AGENTS.md` | 6KB | **규칙.** 값은 토큰에서 · 아이콘은 `.i-*` · 테두리는 border · 여백은 감싸는 쪽 … |
| `llms.txt` | 1.5KB | 어디서 시작하는지 가리키는 표지 |
| `registry.json` | 4.5KB | 컴포넌트 15개 색인 — id · 이름 · 상태 · 한 줄 |
| `registry/<id>.json` | 1~3KB | 그 컴포넌트만. 클래스·상태·쓰는 화면·**실제 조합**·마크업 |
| `registry/아이콘.json` | 6.5KB | 아이콘 64종 이름. `icons.css` 122KB 를 열 필요가 없다 |

처음 읽을 것이 **410KB → 10KB 안쪽**이 된다. 나머지는 필요할 때 낱개로 가져간다.

```
python3 레지스트리-만들기.py         # 컴포넌트를 고친 뒤 돌린다 (쓰임-조사.py 다음)
```

**사실은 생성하고 판단은 쓴다.** `레지스트리-만들기.py` 가 사실(클래스·의존·조합·마크업)을
만들고, `AGENTS.md` 와 `상태.json` 에 판단을 손으로 적는다. 섞으면 둘 다 썩는다.

### 절에 적는 것 셋 — 손으로 적지만 안 바뀐다

| 속성 | 무엇 |
|---|---|
| `data-use-family` | 그 절의 주 가족 (`btn` · `bl` · `drawer` …) |
| `data-use-scope` | 가족을 공유하는 절끼리 범위를 나눌 때 (Board list 세 절) |
| `data-use-ignore` | 카탈로그가 데모를 세우려고 붙인 클래스 (`drawer--inline`) |

## 처음 보는 사람은

**`index.html` 을 연다** — 컴포넌트 카탈로그와 정책 정의서 중에서 고른다(2026-09-10 부터).

| 화면 | 무엇 |
|---|---|
| `카탈로그.html` | 컴포넌트 21종의 모든 변형 + 실측값. **기본은 우리가 쓰는 것만** |
| `정책.html` | 정의서 4장 31문장을 **우리 구현과 대조**. 기본은 **확인 필요**(지금 8건) |

둘 다 들어올 때 **이름과 닉네임**을 넣는다 — 남긴 메모에 이름이 붙는다.
옛 링크(`?memo=…`·`#앵커`)로 오면 예전처럼 카탈로그로 넘어간다.
...
- `../크리스아이티-사실대장.md` — 문구는 여기서만 꺼내 쓴다. 카탈로그에 들어간 회사 정보
  ...
  실제 사용 전에 사실대장과 대조한다.
- `../../0. 본사 홈페이지 리뉴얼/05. 효과 아카이브/안전하게-컴포넌트.html` — 모션 컴포넌트.
  이 폴더의 정적 컴포넌트와 별개다. 2026-09-10 에 그 폴더로 옮겼다.
