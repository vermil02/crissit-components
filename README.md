...
카탈로그에는 값의 출처와 판단 근거를 적은 글이 컴포넌트마다 붙어 있다
(`.desc` `.src` `.note` `.spec`). 개발할 때는 그게 본체지만 **모양만 보려는
사람에게는 방해**라, 오른쪽 아래 **「설명」** 버튼으로 껐다 켠다.

- **기본은 꺼짐**이다. 처음 여는 사람은 컴포넌트만 깔끔하게 본다
- 선택은 브라우저에 남아서 페이지를 옮겨도 유지된다
- 숨기지 않는 것 — 절 제목·카드 제목·표·행 이름(`Disable = False / True` 같은 것).
  그건 설명이 아니라 무엇을 보여주는지 알려주는 내용이다
...
- 메모는 검사 패널의 **오른쪽 열**에 따로 있다. 값을 보면서 메모를 쓸 수 있게 갈라 뒀다 —
  화면이 1600px 보다 넓으면 패널이 680px 로 늘어나 좌우로 나란히 서고, 좁으면 아래로 쌓인다
