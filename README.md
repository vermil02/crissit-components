...
`카탈로그.html` 의 css·js 주소에 `?v=20260908a` 가 붙어 있다. **깃허브 Pages 가
css·js 를 캐시해서**, 고쳐 올려도 이미 열어 본 브라우저는 옛 파일을 계속 쓴다 —
강제 새로고침(`cmd+shift+r`)을 해야 바뀐다. 실제로 답글 기능을 올렸는데
"안 보인다" 는 이야기가 나온 원인이 이것이었다.

주소가 달라지면 브라우저가 새로 받으므로, **css·js 를 고칠 때마다 그 값을 바꾼다.**
날짜 + 그날의 순번(`20260908a` → `20260908b`)으로 쓰고 있다. 21곳이 같은 값을 쓰므로
찾아 바꾸기로 한 번에 고친다.

```
grep -o '?v=[0-9a-z]*' 카탈로그.html | sort -u        # 지금 값 확인
```

`assets/*.svg` 에는 붙이지 않았다. 아이콘·로고는 거의 안 바뀌고, 바꿀 때는
파일 이름이 같아도 CSS 가 새로 받아지므로 대개 따라온다. 아이콘을 바꿨는데
옛 그림이 보이면 그때 강제 새로고침한다.
...
지우기가 되돌릴 수 없는 일이면 사람들이 지우기를 아예 안 쓴다. 그래서 지운
메모는 **휴지통**에 담긴다 — 목록 아래 **「휴지통 N」**에서 「되살리기」 할 수 있고,
최대 100개까지 남는다. 되살리면 내 메모로 돌아가고, 서버에 붙어 있으면 다시 올라간다.

- **「내 메모 비우기」** — 이 브라우저의 내 메모만 비운다. **서버는 건드리지 않는다.**
  서버에 올라간 것은 「공유」 메모로 다시 내려오므로 잃는 게 없다
- **한 건 「삭제」** — 서버에서도 지운다. 그래도 휴지통에는 남는다
- **「휴지통 비우기」** — 이건 정말 지운다

상대방 브라우저를 정리해 주고 싶으면 이 링크를 보내면 된다 — 열기만 하면
그 브라우저의 내 메모가 휴지통으로 들어간다(서버는 그대로).

```
https://vermil02.github.io/crissit-components/?memo-reset=1
```

### 남이 쓴 메모는 알아서 내려온다

서버에 붙어 있으면 **30초마다, 그리고 이 탭으로 돌아올 때마다** 확인한다 —
새로고침할 필요가 없다. 배경 탭에서는 쉬고, **메모를 쓰는 중에는 화면을 갈지
않는다**(목록을 다시 그리면 입력 중인 글이 날아간다). 바뀐 게 없으면 아무것도
다시 그리지 않는다.
