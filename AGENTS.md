...
**사람이 보는 정책 화면**은 `정책.html` 이다(허브 `index.html` 에서 고른다).
정의서 문장을 우리 구현과 나란히 놓고 `지킴 / 다르게 정함 / 미구현 / 일부 / 미확인` 으로
갈라 둔다. 기본 보기는 **확인 필요**다. 2026-09-10 조사에서 **31문장 중 8건이 확인 필요**였다.
...
| 정책 대조 화면 | `정책.html` ← `03. 페이지 소스화/정책-페이지-만들기.py` | 34KB — 사람이 본다 |
| 들어오는 자리 | `index.html` | 카탈로그 · 정책을 고른다 |
