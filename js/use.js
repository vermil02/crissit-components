  ...
  function run(data) {
    ...
    function judge(el, fam, ignore) {
      ...
      for (var i = 0; i < own.length; i++) {                 // ① 클래스가 어디에도 없나
        ...
        if (!g) return { off: true, 이유: own[i] + " 를 쓰는 화면이 없다" };
        if (g === "확정아님") best = "확정아님";
      }
      ...
      if (!known[sig]) {
        ...
        if (sup) return { off: true, 이유: "화면은 늘 <code>." + sup.split(" ").pop() + "</code> 를 함께 쓴다" };
      }
      return best;
    }
    ...
    document.querySelectorAll("section.sec").forEach(function (sec) {
      ...
      sec.querySelectorAll(".iconset > span").forEach(function (span) {
        ...
        if (why) span.setAttribute("data-use-why", why);      // 이유도 같이 옮긴다
      ...
      sec.querySelectorAll(".card, .frame").forEach(function (card) {
        ...
        if (!on && !soon) {
          ...
          var why = (card.querySelector("[data-use-why]") || {}).getAttribute
                  ? card.querySelector("[data-use-why]").getAttribute("data-use-why") : "";
          if (h3) h3.appendChild(tag("off", "안 씀"));
          /* 이유는 `.use-why` — 접힘 안내(`.use-hidden`)와 달리 「전부」 모드에서도 보인다.
             흐려진 판을 보고 있는 사람에게 필요한 것은 「왜」이기 때문이다 */
          if (h3 && why) h3.insertAdjacentHTML("afterend",
            '<p class="use-why">어느 화면도 안 씁니다 — ' + why + '.</p>');
          return;
        }
        if (off) {
          if (h3) h3.appendChild(tag("off", "안 쓰는 변형 " + off + "개"));
          var p = document.createElement("p");
          p.className = "use-hidden";
          p.innerHTML = "안 쓰는 변형 <b>" + off + "개</b>를 접었습니다. " +
            '<button type="button" data-use-mode="all">전부 보기</button>';
          card.appendChild(p);
        }
      });
    ...
    bar.querySelector("[data-use-count]").innerHTML =
      "화면이 쓰는 것 <b>" + (tally.on + tally.soon) + "</b>" +
      " · 안 쓰는 것 <b>" + tally.off + "</b>" +
